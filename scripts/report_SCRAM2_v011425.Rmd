...
<!-- Modified: 
29 Apr 24 - fixed annual estimate in final figure to be median and minor caption changes 
01 Jan 25 - Intermittent issue with report generation for some users - log file suggests: 
! Missing $ inserted.
 <inserted text> 
                 $
l.140 \newpage
_" and "#" not allowed in text input for Markdown for conversion to pdf - fixed prior to bringing into markdown
-->
...
<br>
<br>
Note about tracking data inputs: the SCRAM2 movement model uses automated radio telemetry (Motus) data for Red Knots, Piping Plovers, and Roseate Terns, as well as data from GPS and Argos transmitters for Red Knots. Sample sizes are n=240 Red Knots (Motus), n=106 Red Knots (GPS/Argos), n=107 Piping Plovers (Motus), and n=134 Roseate Terns (Motus). Due to when usable tracking data were collected in the study area, collision risk predictions using tracking data are provided for the months of August to November (Red Knots) and May-August (Piping Plovers and Roseate Terns). The Band 2012 Annex 6 model does not use tracking data to help estimate occupancy; instead, it assumes an even distribution of birds throughout a predefined migratory front. Thus, it estimates collisions for all 12 months of the year. For more information, see reports available at https://www.collisionrisk.org/scram.
<br>
<br>
...
if (params$migration_calc_type == "occup_model") {
 fig1_cap_txt <- "A map of the mean monthly species occurrence probabities (i.e., the mean of all summed daily occurrence probabilities across all months) and wind farm location used for SCRAM V2 occupancy modeling calculations. Collision estimates use summed daily occurrence probability rather than these values as shown; the values in this figure are presented for display purposes only to show relative differences in occurrence across the area of interest."
} else {
 fig1_cap_txt <- "A map of the  wind farm location and migratory front used for SCRAM V2 Band Annex 6 calculations. Species density (birds/km) along the front is calculated as the number of migrating animals in a month divided by the length of the migratory front (km)."
}

```


```{r, fig1, fig.height = 7.5, echo=FALSE, warning=FALSE, message = FALSE, fig.cap = fig1_cap_txt, fig.pos="t"}
...
if(params$migration_calc_type == "occup_model") {
  ...
  spp_move_occur$quant <- as.factor(quants(spp_move_occur$mean_monthly[,params$model_type]))
  # spp_move_occur$quant_labels <- quants_labels(spp_move_occur$mean_daily_allmonths)
  spp_move_occur$quant_labels <- quants_labels(spp_move_occur$mean_monthly[,params$model_type])
...
# if(params$migration_calc_type == "occup_model") {
  ...
  for(s in 1:num_species){
    ...
    for(t in 1:num_turb_mods){
      ...
      daily_cell_summary <- spp_daily_cell_count_melt %>%
        ...
        dplyr::summarise(median = formatC(median(daily_cell_count, na.rm = T), digits=4, format="g"),
                         lower = formatC(quantile(daily_cell_count, c(0.025), na.rm=T), digits=4, format="g"),
                         upper = formatC(quantile(daily_cell_count, c(0.975), na.rm=T), digits=4, format="g")) %>%
      ...
      #calculate median and 95% CI for daily collisions
      daily_collisions_summary <- daily_collisions_melt %>%
        dplyr::group_by(species, turbine_model, month) %>%
        dplyr::summarise(median = formatC(median(coll_rate, na.rm = T), digits=4, format="g"),
                         lower = formatC(quantile(coll_rate, c(0.025), na.rm=TRUE), digits=4, format="g"),
                         upper = formatC(quantile(coll_rate, c(0.975), na.rm=TRUE), digits=4, format="g")) %>%
        dplyr::mutate_if(is.character, str_replace, pattern = "NaN", replacement = "") %>%
        dplyr::mutate_if(is.character, str_replace, pattern = "NA", replacement = "") %>%
  ...
  if (params$migration_calc_type == "occup_model") {
 tbl_cap_txt <- "The populations estimate for each month and the median estimated daily number of (95 perc. prediction intervals) animals in the model cell and collisions at the wind farm using SCRAM V2 occupancy modeling calculations. Results are not shown for months that do not have movement data. This does not mean that collisions could not occur in those months, but we do not have movement data to estimate collisions during these periods."
} else {
 tbl_cap_txt <- "The populations estimate for each month and the median estimated daily number of (95 perc. prediction intervals) animals in the model cell and collisions at the wind farm using SCRAM V2 Band Annex 6 calculations."
}
  
  kbl(model_summary_out, booktabs=T, align = "lcccc", digits = 32, caption = tbl_cap_txt) %>%
    kable_styling(position="left", full_width=F, latex_options = c("HOLD_position", "striped")) %>%
...
#calculate annual summary of model runs, median of sum of posteriors and then quantiles of these sums for predict interval
...
annual_rate <- annual_sum %>% 
  ...
  dplyr::summarise(median = formatC(median(sum_monthly, na.rm = T),digits=4, format="g"), 
                   lower = formatC(quantile(sum_monthly, c(0.025), na.rm=TRUE), digits=4, format="g"),
                   upper = formatC(quantile(sum_monthly, c(0.975), na.rm=TRUE), digits=4, format="g"))  %>% 
  dplyr::mutate(turbine_model = sub("turbModel", "", turbine_model)) %>% 
  dplyr::mutate_if(is.character, str_replace, pattern = "NaN", replacement = "") %>%
  dplyr::mutate_if(is.character, str_replace, pattern = "NA", replacement = "") %>%
...
if (params$migration_calc_type == "occup_model") {
 tbl_cap_txt <- "The estimated monthly median number (95 perc. prediction intervals) of collisions using SCRAM V2 occupancy modeling calculations. Results are not shown for months that do not have movement data and does not mean that collisions could not occur in those months."
} else {
 tbl_cap_txt <- "The estimated monthly median number (95 perc. prediction intervals) of collisions using SCRAM V2 Band Annex 6 calculations."
}
...
if (params$migration_calc_type == "occup_model") {
 fig2_cap_txt <- "A frequency histogram of the total number of collisions per year determined using SCRAM V2 occupancy modeling calculations. The heights of the bars show the relative frequency of each value. Months for which movement data were provided or available are shown in bold; only bold months are shown in histogram of annual collisions."
} else {
 fig2_cap_txt <- "A frequency histogram of the total number of collisions per year determined using SCRAM V2 Band Annex 6 calculations. The heights of the bars show the relative frequency of each value. Months for which movement data were provided or available are shown in bold; only bold months are shown in histogram of annual collisions."
}

```

```{r, fig2, fig.height = 4, echo=FALSE, warning=FALSE, message = FALSE, fig.cap = fig2_cap_txt, fig.pos="t"}
...
for(s in 1:num_species){
  for(t in 1:num_turb_mods){
    ...
    if(!is.null(monthly_collisions)){
      ...
      p2 <- cowplot::ggdraw(cowplot::add_sub(p1, label = c("Month", "( Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec )"), x = seq(0,0.9,0.9/12), color = month_col, size = 8, fontface = bold))
      print(p2)
...
if (params$migration_calc_type == "occup_model") {
 fig3_cap_txt <- "The predicted median and 95 perc. prediction intervals of the number of collisions per month using SCRAM V2 occupancy modeling calculations. Results are not shown for months that do not have movement data. Total annual collision rate and 95 perc. prediction interval are given at top. The threshold is shown divided by the number of months that movement data were available."
} else {
 fig3_cap_txt <- "The predicted median and 95 perc. prediction intervals of the number of collisions per month using SCRAM V2 Band Annex 6 calculations. Total annual median collision rate and 95 perc. prediction interval are given at top. The threshold is shown divided by the number of months that movement data were available."
}

```

```{r, fig3, fig.height = 4, echo=FALSE, warning=FALSE, message = FALSE, fig.cap=fig3_cap_txt} 
...
for(s in 1:num_species){
  for(t in 1:num_turb_mods){
    ...
    annual_rate <- annual_summary %>% 
      dplyr::mutate(mean = formatC(median(annual_summary$sum_monthly, na.rm=TRUE), digits=3, format="g"),
    ...
    annual_collision_rate_txt <- paste0("Total est. annual num. of collisions (95 perc. prediction interval): ", annual_rate$mean, 
