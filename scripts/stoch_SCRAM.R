...
#' @importFrom stats sd
#' @export
stoch_SCRAM <- function(model_options = c('1', '2', '3', '4'),
                        n_iter = 100,
                        migrant_resident = "migrant",
                        flt_speed_pars,
                        body_lt_pars,
                        wing_span_pars,
                        avoid_bsc_pars = NULL,
  ...
  if(verbose) cli::cli_h2("Stochastic CRM for SCRAM")
  ...
  if(!is.null(log_file)){
    if(is.character(log_file) & nchar(log_file)>0){
      ...
      # Send message to log
      logr::log_print(paste("Stochastic CRM for SCRAM run details",
                            "---------------------------",
            ...
            dplyr::summarise(
              dplyr::across(dplyr::everything(),
                            list(mean=mean, sd=sd, median = median,
                                 `pctl_2.5` = ~quantile(.x, 0.025, na.rm=T), 
                                 #add na.rm=T due to missing values and NaN's not allowed if 'na.rm' is FALSE
                                 `pctl_97.5` = ~quantile(.x, 0.975, na.rm=T)),
                            .names = "{.col}---{.fn}")
            ) %>%
            tidyr::pivot_longer(cols = dplyr::everything()) %>%
            tidyr::separate("name", sep = "---", into = c("period", "stat")) %>%
