...
generate_rotor_grids_SCRAM <- function(yinc = 0.01, xinc = 0.01, chord_prof) { #Change from 0.05 to account for larger turbines
  
  # horizontal distances from vertical axis of rotor's circle
  x_grid <- get_x_grid_SCRAM(yinc, xinc)
  
  # vertical distances from horizontal axis of rotor's circle
  y_grid <- get_y_grid_SCRAM(x_grid, yinc)
  
  # angles (in radians) between grid cells and the rotor vertical axis
  phi_grid <- get_phi_grid_SCRAM(x_grid, y_grid)
...
#'
#' @export
get_x_grid_SCRAM <- function(xinc = 0.01, yinc = 0.01){ #Change from 0.05 to account for larger turbines
  ...
  for(i in length(y_i):1){
    ...
    if(xmax == 0){
      ...
      if(xmax == 1){
        ...
        # Fixed where xinc should be inserted
...
#'
#' @export
get_y_grid_SCRAM <- function(x_grid, yinc = 0.01){  #changed from default 0.05
...
#' @export
get_phi_grid_SCRAM <- function(x_grid, y_grid){
