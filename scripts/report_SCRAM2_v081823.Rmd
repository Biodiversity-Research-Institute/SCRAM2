...
# load(file = "scripts/test_params.RData")
...
if(params$migration_calc_type == "occup_model") {
  ...
  model_txt <- paste0("Migration front type: ", params$migr_front_type, "\n",
...
if(params$model_option == "1") {
  params_list <- c("avoid_bsc", "wing_span", "body_lt", "flt_speed")  
  tbl_names <- c("Species", "Turbine model", "Avoidance basic", "Wing span", "Body length", "Speed")
} else if(params$model_option == "3") {
  params_list <- c("avoid_ext", "wing_span", "body_lt", "flt_speed")  
  tbl_names <- c("Species", "Turbine model", "Avoidance extd.", "Wing span", "Body length", "Speed")
}
...
colnames(turb_params_table2) <- c("Species", "Turbine model","Rotor radius", "Air gap (m)", "Hub height (m)", "Blade width (m)", "Rotor speed (rpm)", "Pitch (radians)")
...
for(s in 1:num_species){
  for(t in 1:num_turb_mods){
    ...
    z = 1
    for(month in 1:6){
      turb_params_table_pt1[index_a, (z + 2)] <- paste0(round(prop_oper[month, "mean"], 3), " (", 
                                                    round(prop_oper[month, "pctl_2.5"], 3), ", ", 
                                                    round(prop_oper[month, "pctl_97.5"], 3), ")")
      z = z + 1
    }
  }
}
...
for(s in 1:num_species){
  for(t in 1:num_turb_mods){
    ...
    z = 1
    for(month in 7:12){
      turb_params_table_pt2[index_a, (z + 2)] <- paste0(round(prop_oper[month, "mean"], 3), " (", 
                                                    round(prop_oper[month, "pctl_2.5"], 3), ", ", 
                                                    round(prop_oper[month, "pctl_97.5"], 3), ")")
      z = z + 1
    }
...
if(params$migration_calc_type == "occup_model") {
  ...
  for(s in 1:num_species){
    ...
    for(t in 1:num_turb_mods){
      ...
      spp_daily_cell_count_melt <- reshape2::melt(spp_daily_cell_count, value.name = "daily_cell_count") %>% 
        mutate(species = model_out$species, turbine_model = model_out$turbine_model) %>% 
      ...
      daily_cell_summary <- spp_daily_cell_count_melt %>%
        ...
        dplyr::summarise(mean = formatC(mean(daily_cell_count, na.rm = T), digits=3, format="G"),
                         lower = formatC(quantile(daily_cell_count, c(0.025), na.rm=TRUE), digits=3, format="G", ),
                         upper = formatC(quantile(daily_cell_count, c(0.975), na.rm=TRUE), digits=3, format="G")) %>%
  ...
  kbl(model_summary_out, booktabs=T, align = "lcccc",
