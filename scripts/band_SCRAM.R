...
#'
#' @export
band_SCRAM <- function(model_options = c('1', '2', '3', '4'),
                       migrant_resident,
                       flight_speed,
                       body_lt,
                       wing_span,
  ...
  if (any(model_options %in% c('3', '4'))) {
    if (is.null(avoid_rt_ext)) {
      stop("Missing argument value: `avoid_rt_ext` needs to be provided if
           `model_options`comprise values '3' and/or '4'. Calculations under the
  ...
  if (migrant_resident == "migrant") {
    ...
    flux_fct <-
      get_mig_flux_SCRAM(
        n_turbines = n_turbines,
        rotor_radius = rotor_radius,
        density_est = dens_month
      )
  } else {
    #resident bird flux factor - typical flux calcs
    #this is the function from stochLab
    flux_fct <-
      get_flux_factor(
        n_turbines = n_turbines,
        rotor_radius = rotor_radius,
