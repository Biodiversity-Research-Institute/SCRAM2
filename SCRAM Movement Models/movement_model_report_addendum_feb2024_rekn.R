...
## subset to fall migratory (southbound) flights based on Loring et al. 2021 ####
allindvs <- allindvs[format(allindvs$ts_gmt,"%m-%d") > "06-21",] #remove spring trajectories in data to retain only fall migration
str(allindvs)
...
### FILTERING ####
## 2021 data pre-filtered for false positives, duplicate bursts, and stationary individuals ###################################

#Filters already incorporated into 2021 Loring et al. 2021: 
# "For this dataset we filtered data from all receivers using a consistent cutoff (minimum 7 consecutive bursts)."
# "We removed 52 birds from the dataset that had ambiguous detections. 
# Ambiguous detections occur when multiple tags with the same properties (ID code, burst interval) 
# are active in the Motus network at the same time and cannot be distinguished from each other. 
# We then filtered out detections with < 3 consecutive bursts and where the standard deviation in 
# transmitting frequency was > 0.1 kHz (Mckinnon et al. 2019). 
# Some receiving stations in the Motus network are prone to high rates of false positive detections 
# (Loring et al. 2018; Anderson et al. 2019). 
# We therefore used a stricter filter, removing detections with < 7 consecutive bursts, 
# for detection data from the broader Motus network as well as one BOEM receiving station (JMBY) 
# that was situated in a noisy urban location within New York, NY. 
# Remaining false detections were identified by examining flight speeds between receiving stations and 
# mapping flight paths. 
# Flights were considered suspect if flight speed was > 42 m/s (Grӧnroos et al. 2012) 
# or the flight path showed a large movement in a direction inconsistent with typical migration 
# (e.g. a movement of 500 km north during southbound migration). 
# We further examined the detections associated with these suspect flights for 
# other indications that they were false (suspect detections occurring close in time 
# with more reliable detections at a distant receiving station; 
# short runs of consecutive bursts, no detections at nearby receiving stations). 
# Through this process we found that some receiving stations were consistently associated with 
# high rates of false positives during certain time periods and we therefore removed all detections 
...
# locations
allindvs_ordered_filtered_nodups$lat <- allindvs_ordered_filtered_nodups$recvLat
allindvs_ordered_filtered_nodups$lon <- allindvs_ordered_filtered_nodups$recvLon
